...
/**
 ** @function void *thread_pool_thread(void *thread_pool)
 ** @desc the worker thread
 ** @param thread_pool the pool which own the thread
 **/
void* thread_pool_thread(void* thread_pool);

/**
 ** @function void *adjust_thread(void *thread_pool);
 ** @desc manager thread
    ...
    do 
    {
        ...
        for (i = 0; i < min_thr_num; i++)
        {
            pthread_create(&(pool->threads[i]), NULL,
                thread_pool_thread, (void*)pool);/*pool指向当前线程池*/
            printf("start thread 0x%x...\n", (unsigned int)pool->threads[i]);
        }
        pthread_create(&(pool->adjust_tid), NULL,
            adjust_thread, (void*)pool);/* 启动管理者线程 */

        return pool;

    } while (0);

    thread_pool_free(pool);      /* 前面代码调用失败时，释放poll存储空间 */
...
void* adjust_thread(void* thread_pool)
{
    ...
    while (!pool->shutdown) {
        ...
        /* 创建新线程 算法： 任务数大于最小线程个数, 且存活的线程数少于最大线程个数时 如：30>=10 && 40<100*/
